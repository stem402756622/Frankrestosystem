        ...
        if (!$table_id) {
            ...
            if ($role === 'customer') {
                ...
                if ($pref && $pref['preferred_table_id']) {
                    $tbl = db()->fetchOne("SELECT * FROM restaurant_tables WHERE table_id=? AND status='available' AND capacity>=?", [$pref['preferred_table_id'], $party_size]);
                    if ($tbl) $table_id = $tbl['table_id'];
                }
            }
            // Auto-assign best available
            if (!$table_id) {
                $best = db()->fetchOne("SELECT table_id FROM restaurant_tables WHERE status='available' AND capacity>=? ORDER BY capacity ASC LIMIT 1", [$party_size]);
                if ($best) $table_id = $best['table_id'];
            }
        }
