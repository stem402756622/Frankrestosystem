-- Frank Restaurant System - Complete Database Update Script
-- This script contains all SQL statements needed to implement the 18 missing features
-- Execute these commands in your MySQL management tool (phpMyAdmin, MySQL Workbench, etc.)

USE frank_restaurant;

-- Feature 1: Reschedule Limits
ALTER TABLE reservations ADD COLUMN IF NOT EXISTS reschedule_count INT DEFAULT 0 AFTER status;

-- Feature 2: Email Notifications
ALTER TABLE reservations ADD COLUMN IF NOT EXISTS is_reminded TINYINT(1) DEFAULT 0;
...
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS dietary_tags SET('Vegetarian','Vegan','Gluten-Free','Halal','Spicy') DEFAULT NULL;
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount_amount DECIMAL(10,2) DEFAULT 0 AFTER tax;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS promo_code_id INT DEFAULT NULL AFTER discount_amount;
